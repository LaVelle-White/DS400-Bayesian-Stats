...
```{r}
# 1) Clean data (drop rows with missing essentials)
...
#2) Visualize cluster: bill lenght vs bill depth
p <- ggplot(penguins_clean,
            aes(x = bill_length_mm, y = bill_depth_mm,
                color = species, shape = island)) +
  geom_point(alpha = 0.8) +
  stat_ellipse(level = 0.8, linewidth = 0.7) +
  labs(title = "Penguin species clusters",
       subtitle = "bill_length_mm vs bill_depth_mm",
       x = "Bill length (mm)", y = "Bill depth (mm)",
       color = "Species", shape = "Island") +
  theme_minimal(base_size = 12)
print(p)

# 3) Compute species centroids in this 2D space
centroids <- penguins_clean %>%
  group_by(species) %>%
  summarise(
    bl = mean(bill_length_mm, na.rm = TRUE),
...
```{r}
# 4) Enter your unknown penguin's measurements (REPLACE the numbers)
unknown <- data.frame(
  bill_length_mm = 45.0,  # <-- REPLACE_ME
  bill_depth_mm  = 18.0   # <-- REPLACE_ME
)

# Overlay the unknown on the cluster plot
p_unknown <- p +
  geom_point(data = unknown, aes(bill_length_mm, bill_depth_mm),
             inherit.aes = FALSE, size = 4, color = "black", shape = 8) +
  annotate("text",
           x = unknown$bill_length_mm,
           y = unknown$bill_depth_mm,
           label = "Unknown",
           vjust = -1, color = "black")
print(p_unknown)

# 5) Nearest-centroid classification (Euclidean distance in BL/BD)
distances <- centroids %>%
  rowwise() %>%
  mutate(dist = sqrt((bl - unknown$bill_length_mm)^2 + (bd - unknown$bill_depth_mm)^2)) %>%
  arrange(dist) %>%
  ungroup()

predicted_species <- distances$species[1]
predicted_species
distances   # see the full ranking
...
```{r}
our_penguin <- data.frame(bill_length_mm = 50, flipper_length_mm = 195, above_average_weight = "under_avarage_weight")
```
...
df_nation_pulse <- pulse_of_the_nation
```

```{r}
#Fit naive Bayes
my_model <- naiveBayes(
  climate_change ~ wise_unwise + science_is_honest, pulse_of_the_nation)
...
```{r}
our_person <- data.frame(wise_unwise = "Wise but Unhappy", science_is_honest = "Strongly Agree")
```

```{r}
predict(my_model, newdata = our_person, type = "raw")
```
