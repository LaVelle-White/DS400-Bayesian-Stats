...
#-------------------- Relationship Streght Chart -----------------------

# pick only the parametric fixed effects (smooth of doy is not a single slope)
terms <- c("b_temp_z","b_pH_z","b_temp_z:pH_z","b_hour_sin","b_hour_cos")

draws <- as_draws_df(fit_med_ar1) %>%
...
ggplot(strength, aes(x = abs_mean, y = term)) +
  geom_point() +
  geom_errorbarh(aes(xmin = abs(lo), xmax = abs(hi)), height = 0) +
  labs(
    x = "Effect size (|posterior mean| on standardized scale)",
    y = NULL,
    title = "Which predictors have the strongest association with oxygen?"
  ) +
  theme_minimal()
